...
def get_data(csv):
    if csv:
        ...
        timestamps = preprocess.get_timestamps(
            preprocess.config.yrs, preprocess.config.mths, preprocess.config.dys
        )
...
def analyse_movement(df):
    ...
    dates = [
        str(seas_d.trend.index.values[i]).split("T")[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "symbol",
        type=str,
        help="the stock symbol you want to Analysis, e.g. MSFT,AAPL,GOOG",
    )
    args = parser.parse_args()
    csv_file_path = f"./Data/{args.symbol}.csv"  # stock_csv_filepath
