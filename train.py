...
def plot_predictions(df, train_data_size, predictions, model_name: str):
    ...
    stock_symbol = valid["Company stock name"].values[0]
    valid.to_csv(
        f"./{stock_symbol}_price_predictions.csv"
    )

    print("RMSE:", stock_symbol, model_name, RMSE)

    fig.add_trace(
        go.Scatter(
            x=x_val[-100:],
            y=valid["Adj Close"][-100:],
            mode="lines+markers",
            line_color=colors[1],
            line_width=1,
    ...
    fig.update_layout(
        title=dict(
            text=f'Predictions of stock "{train["Company stock name"][0]}" from {x_val[0]} to {x_val[len(valid) - 1]}',
            xanchor="auto",
        ),
    ...
    fig.write_image(
        f"./demonstration_images/{stock_symbol}_{model_name}_predictions.svg"
    )
    fig.show()
