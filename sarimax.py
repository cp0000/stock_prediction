...
def get_train_test(df, train_split, scale=True, stationary=False):
    ...
    if scale:
        scaled_input = sc_in.fit_transform(
            df[["Low", "High", "Open", "Close", "Volume", "Adj Close", "Mean"]]
        )
        ...
        stationary_data = Analysis.get_stationary_data(
            df, df.columns if df.columns[0] != "Date" else df.columns[1:], 12
        )
        X = stationary_data[
...
def predict_arima(
    ...
    predictions = results.predict(
        start=len(train_X), end=len(train_X) + len(test_X) - 1, exog=test_X
    ).values
    forecast = results.forecast(steps=len(test_X), exog=test_X)
    act_arr = np.array(df_orig["Actual"][-len(test_X) :].values)
    if scale:
        act_arr = sc_out.inverse_transform(
            np.array(scaler_output.iloc[len(train_X) :, 0].values).reshape(-1, 1)
        )
...
def plot_predictions(predictions, stock_name: str):
    ...
    RMSE = np.sqrt(
        mean_squared_error(
            predictions["Predictions"].values, predictions["Actual"].values
        )
    )
    predictions["Actual"].plot(
        figsize=(20, 8), legend=True, color="#81A6FF", linewidth=1
    )
    predictions["Predictions"].plot(
        legend=True, color="#C281FF", linewidth=1, figsize=(20, 8)
    )
    plt.ylabel("USD $")
...
if __name__ == "__main__":
    ...
    predictions = predict_arima(
        results, test_X, train_X, scaler_output, sc_out, df_orig, scale=False
    )
