    ...
    def forward(self, query, key, value):
        multiheads_out = [
            scaled_dot_product_attention(
                self.query(query), self.key(key), self.value(value)
            )
...
def positioning_encoding(seq_length, model_dim):
    ...
    frequencies = 1e-4 ** (
        2 * (torch.arange(model_dim, dtype=torch.float) // 2) / model_dim
    ).reshape(1, 1, -1)
...
def forward(input_dim=512, forward_dim=2048):
    ...
    forward_layer = nn.Sequential(
        nn.Linear(input_dim, forward_dim), nn.ReLU(), nn.Linear(forward_dim, input_dim)
    )
...
class Encoder(nn.Module):
    ...
    def __init__(
        self, n_layers=6, model_dim=512, num_heads=8, forward_dim=2048, dropout=0.2
    ):
        super().__init__()
        ...
        self.feed_forward = ResidualConnection(
            forward(model_dim, forward_dim), dimension=model_dim, dropout=dropout
        )
